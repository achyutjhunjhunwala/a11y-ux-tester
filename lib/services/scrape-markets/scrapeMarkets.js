...
class MarketScrapper {
  ...
  async getTextDetails() {
    ...
    //   logger.info('sleeping 100ms'); //  sleep to eliminate race condition
    //   setTimeout(function () {
    //     logger.info(`Browser Disconnected... Process Id: ${process}`);
    //     exec(`kill -9 ${process}`, (error, stdout, stderr) => {
    //       if (error) {
    //         logger.info(`Process Kill Error: ${error}`)
    //       }
    //       logger.info(`Process Kill Success. stdout: ${stdout} stderr:${stderr}`);
    //     });
    //   }, 100);
    // });
    ...
    await page.setUserAgent('44273f28-903c-47b3-b0ec-bd5cac6d4313');

    let scrappedData = {};

    logger.info(`ScrapeMarkets: Starting Puppeteer for ${this.url}`);

    // page.on('console', msg => logger.info('PAGE LOG:', ...msg.args));
  ...
  async instantiateHelpers(page) {
    const instance = await page.evaluate(() => {
      ...
      window.getAverageColourAsRGB = async (sourceImage, textArea) => {
        ...
        const palette = window.getPalette(sourceImage, 5, 1, textArea);
      ...
      */
      ...
      window.getColor = (sourceImage, quality, textArea) => {
        const palette = window.getPalette(sourceImage, 5, quality, textArea);
